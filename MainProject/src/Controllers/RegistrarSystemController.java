...
public class RegistrarSystemController extends Controller {
	...
	private void deleteStudentEvent() throws Exception {
		...
		int deleteOption = JOptionPane.showOptionDialog(rv.getFrame(), "Are you sure that you would like to delete "+selectedStudent.getFirstName()+" "+selectedStudent.getSecondName()+" from the database?",
				"Delete question", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE, null, options, options[0]);
	...
	public Object[][] getStudentsData() throws Exception {
		...
		for (int i = 0; i < results.size(); i++) {
			students.add(new Student(Integer.parseInt(results.get(i)[0]),results.get(i)[1],results.get(i)[2],results.get(i)[3],results.get(i)[4], results.get(i)[5], results.get(i)[6], results.get(i)[7].charAt(0), Integer.parseInt(results.get(i)[8]), results.get(i)[10], results.get(i)[11]));
	...
	public ArrayList<Module> getAvailableModules() throws Exception {
		...
		if (results.size() > 0) {
			...
			for (String[] result : results) {
				...
				newModule.setCode(code);
				if (result[1] == "0")
					newModule.setCore("false");
				else
					newModule.setCore("true");
